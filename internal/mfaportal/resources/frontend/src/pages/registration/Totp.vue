...
import { ref } from "vue";
import {} from "vue-toastification";
...
import { getTotpDetails, registerTotp } from "@/api";
import { useWebSocketStore } from "@/store/info";

const infoStore = useWebSocketStore();

const { data: registration, isLoading: isLoadingTotpRegistrationDetails } =
  useApi(() => getTotpDetails());
...
<template>
  ...
  <template v-else>
    <h3 class="text-2xl font-bold text-center mb-8">
      Register Two-Factor Authentication Code
    </h3>
    ...
    <div
      ...
      class="mt-4 mb-2 bg-base-200 p-4 rounded-lg text-base-content"
    >
      <p class="mb-2 text-sm" ref="toast">Manually enter these details:</p>
      <div class="grid grid-cols-2 gap-2 text-sm">
        <div class="font-semibold">Type of key:</div>
        <div>Time based</div>

        <div class="font-semibold">Account name:</div>
        <div class="font-mono bg-base-300 px-2 py-1 rounded text-base-content">
          {{ registration?.account_name }}
        </div>

        <div class="font-semibold">Your key:</div>
        <div
          class="font-mono bg-base-300 px-2 py-1 rounded overflow-x-auto text-base-content"
        >
          {{ registration?.key }}
        </div>
      </div>
    ...
    <DigitInput
      execution-name="Verify & Complete Registration"
      @submit="register"
      :loading="isLoading"
    ></DigitInput>
