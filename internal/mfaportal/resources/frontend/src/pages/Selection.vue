...
<template>
  <template v-if="info.isConnected">
    <template
      v-if="
        info.isRegistered &&
        !info.availableMfaMethods.some(
          (x) => x.method === info.state.userInfo?.user_mfa_method,
        )
      "
    >
      <h4 class="mb-2 text-center">
        Your registered MFA method has been disabled.
      </h4>
      <div>
        Please contact your administrator
        <a :href="'mailto:' + info.helpMail">{{ info.helpMail }}</a>
      </div>
    </template>
    ...
    <template v-else>
      <h4 class="text-2xl font-bold text-center mb-4">
        {{ !info.isRegistered ? "Register Authentication Method" : "Sign In" }}
      </h4>
      ...
      >
        <RouterLink
          :to="
            (!info.isRegistered ? '/register/' : '/authorise/') + method.method
          "
        >
