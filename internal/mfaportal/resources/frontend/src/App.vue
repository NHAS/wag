...
function getMFAPath(): string {
  ...
  const methodsHasUserPref =
    info.selectedMFAMethod !== "unset" &&
    info.availableMfaMethods.some((x) => x.method == info.selectedMFAMethod);
  const methodsHasDefault =
    info.defaultMFAMethod != "" &&
    info.availableMfaMethods.some((x) => x.method == info.defaultMFAMethod);
  ...
  if (info.isRegistered && !methodsHasUserPref) {
    ...
    return (
      "/error?m=" +
      encodeURIComponent(
        "User MFA method has been disabled, contact your administrator",
      )
    );
  }
  ...
  try {
    ...
    if (info.availableMfaMethods.length > 0) {
      if (info.isRegistered) {
        ...
        notify(
          "VPN Registration",
          "Please register an MFA method with the VPN",
        );
      }
    }
  } catch (error) {
...
async function stateUpdate() {
  try {
    ...
    if (
      currentState.account_locked != previousState.account_locked ||
      currentState.device_locked != previousState.device_locked
    ) {
      //if we have become locked
      if (isLocked) {
        notify(
          "VPN Locked",
          "Your device has been locked. Please contact help",
        );
        router.push("/locked");
        return;
      }
    ...
    const isAuthorised =
      currentState.is_authorized && currentState.has_registered;
    ...
    if (router.currentRoute.value.name != null) {
      ...
      const isAuthPage =
        currentRouteName.includes("auth") ||
        currentRouteName.includes("register");

      if (isAuthPage) {
        const currentMethods = new Map(
          currentState.available_mfa_methods.map((o) => [o.method, true]),
        );

        // If we are on an mfa method page (registration/authorisation) that has been disabled
        if (
          !currentMethods.has(
            currentRouteName.replace("_auth", "").replace("_register", ""),
          )
        ) {
          router.push(getMFAPath());
          return;
