...
function requirementsMet(selected: actionOption) {
  ...
  return !selected.required.some(s => s.data.value == null) && (selected.required.some(s => s.data.value != null) || selected.optional.some(s => s.data.value != null))
}
    ...
    <div class="w-screen  max-w-[700px]">
      ...
      <div class="mt-8">
        ...
        <div class="max-h-[230px] min-h-[230px]">
          ...
          <div v-if="incommingAttributes.error == ''" class='overflow-y-scroll max-h-[130px]'>
            <table class="table overflow-scroll">
              ...
              <tbody>
                <tr class="hover" v-for="(attribute, index) in filteredAttributes" :key="attribute.key">
          ...
          <EmptyTable v-if="filteredAttributes.length == 0 || incommingAttributes.error != ''"
            :text="incommingAttributes.error == '' ? (incommingAttributes.attributes.length > 0) ? 'Filter doesnt match any attribute' : 'Waiting for webhook input!' : incommingAttributes.error">
          </EmptyTable>
        ...
        <div class="min-h-[300px]">
          ...
          <template v-for="attribute in selectedAction.required">
            <div>
        ...
        <span class="mt-4 flex pt-6">
          <button class="btn btn-primary" :disabled="!requirementsMet(selectedAction)"
            @click="createWebhookTrigger">Create</button>
